...
impl ForecastEngine {
    pub fn new(
        ...
        Self { price_forecaster: price, consumption_forecaster: cons, production_forecaster: prod }
    }

    pub async fn get_forecast_24h(&self, area: PriceArea, household_id: Uuid) -> Result<Forecast24h> {
        let generated_at: DateTime<FixedOffset> = Local::now().fixed_offset();
        let prices = self.price_forecaster.predict_next_24h(area).await?;
        let consumption = self.consumption_forecaster.predict_next_24h(household_id).await?;
        let production = self.production_forecaster.predict_next_24h(household_id).await?;
        Ok(Forecast24h { area, generated_at, prices, consumption, production })
    }
}
